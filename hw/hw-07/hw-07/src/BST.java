...
public class BST {
	...
	public void addLeftSubTreeToMin(BST subtree) {
		...
		BSTNode minOfMain = ((TreeIterator) this.findMin()).currentNode;
		BSTNode maxOfSub = ((TreeIterator) subtree.findMax()).currentNode;
		
		if (maxOfSub.data > minOfMain.data) return;
		
		
		minOfMain.left = subtree.root;
		subtree.root.parent = minOfMain;
		this.size += subtree.size;

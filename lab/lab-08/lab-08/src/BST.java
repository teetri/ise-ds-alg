...
public class BST {
	...
	public int findNextData(int n) {
		// Implement this method 
		int v = n;
		boolean nIsInTree = true;
		if (this.size == 0 || findMax(this.root).currentNode.data <= n) return n;
		// insert n if it is not in the tree
		if (find(n) == null) {
			nIsInTree = false;
			this.insert(n);
		}
		BSTNode temp = ((TreeIterator) find(n)).currentNode;
		// case n has right child
		if (temp.right != null) {
			v = findMin(temp.right).currentNode.data;
		}
		// case n is left child
		else if (temp.parent.left == temp) {
			v = temp.parent.data;
		}
		// case n is right child
		...
		// remove n if its not here in the original tree
		if (!nIsInTree) {
			this.remove(n);
		}
		return v;

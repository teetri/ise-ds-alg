...
//
	...
	public void printList() throws Exception{
		...
		while (itr.hasNext()) {
			...
			System.out.println(String.format("%s %s %s", c.name, c.phone, c.email));
		}
	}
	...
	public void add(Object c) throws Exception{
		...
		while(p.hasNext()) {
			Contact x = (Contact) p.next(); 
			if((x.name).compareTo(((Contact) c).name) > 0) {
				p = (MyListIterator) findPrevious(x);
				break;
			}
		}
		insert((Object)c, p);
	...
	public Iterator removeAt(Iterator i) throws Exception{
		...
		if(itr == null || itr.currentNode == null || itr.currentNode.data == null) {
			return null;
		}
		Iterator p = findPrevious(itr.currentNode.data);
		remove(p);
